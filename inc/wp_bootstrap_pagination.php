...
function seogo_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'seogo' ),
        'next_string'     => esc_html__( '&raquo;', 'seogo' ),
        'before_output'   => '<div class="seogo-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'seogo_pagination_defaults', $defaults )
    );
