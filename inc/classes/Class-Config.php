...
final class Seogo {
	...
	private function setup() {
		...
		$enqueu          = new seogo_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->seogo_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'seogo_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'seogo', SEOGO_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'seogo_site_logo_147x40', 147, 40, true );

		// Banner thumb sizes
		add_image_size( 'seogo_banner_left_circle_605x552', 605, 552, true );
		add_image_size( 'seogo_banner_right_triangle_25x23', 25, 23, true );
		add_image_size( 'seogo_banner_right_circle_679x1251', 679, 1251, true );
		add_image_size( 'seogo_banner_image_920x464', 920, 464, true );

		// Latest post thumbnail size
		add_image_size( 'seogo_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'seogo_single_blog_750x375', 750, 375, true );
		add_image_size( 'seogo_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'seogo' ),
			'services'     => esc_html__( 'Services', 'seogo' ),
			'useful-links' => esc_html__( 'Useful Links', 'seogo' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = SEOGO_DIR_CSS_URI;
		$jsPath  = SEOGO_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'seogo' ) ) {
			
			$font_families = array(
				'Poppins:200,200i,300,300i,400,400i,500,500i,600,600i,700,800',
			);
	...
	private function customizer_init(){
		...
		// Instantiate seogo theme customizer
		$seogo_theme_customizer = new seogo_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'seogo_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'seogo-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'seogo_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'seogo-elementor-default-style',  SEOGO_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->seogo_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'seogo_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'seogo-elementor-notice', SEOGO_DIR_JS_URI.'seogo-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'seogo-elementor-notice',
				'seogoElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'seogo-elementor-notice-nonce' ),
				)
			);
		}
