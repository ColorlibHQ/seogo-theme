...
function seogo_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'seogo' ) . '</span></li>';
...
// Blog categories
function seogo_featured_post_cat(){
...
// social media
if ( ! function_exists( 'seogo_social' ) ) {
	function seogo_social( $args = array() ) {
		...
		$url = seogo_opt( 'seogo_social_url' );
...
// header cart count
function seogo_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( SEOGO_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'seogo' ); ?>">
...
function seogo_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'seogo_contact7_form_content', 10, 2 );